...
function oddValleyIntegrationTest() {
  ...
  describe('Join the wings into a valley', () => {
    test('should be the two wings with the same length', () => {
      const sizeOfTheLeftWing = buildTheValleyComponents(oddValley).left.length;
      const sizeOfTheRightWing = buildTheValleyComponents(oddValley).right.length;
    ...
    test('should be a flat bottom element in the valley', () => {
      ...
      const sumOfTheWingsAndTheBottom =
        buildTheValleyComponents(oddValley).left.length + 1 + buildTheValleyComponents(oddValley).right.length;
